...
async def fetch_company_info(request: CompanyRequest):
    """
    Fetch company information from PitchBook based on the company name provided in the request.

    Parameters:
    - request (CompanyRequest): The request model containing the company name.

    Returns:
    - dict: The content fetched from the company's PitchBook profile or raises an HTTPException.
    """
    company_name = request.company_name  # Extract the company name from the request object
