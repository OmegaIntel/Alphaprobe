...
from services.deep_research.state import SectionState
from services.deep_research.llm import gpt_4
from services.deep_research.prompts import report_planner_query_writer_instructions, query_prompt_for_iteration
from services.deep_research.process_section import (
...
async def node_section_data_needs(state: SectionState) -> SectionState:
    ...
    logger.debug(
        ...
        state.title, state.web_research, state.kb_search, state.excel_search
    )

    # Build dynamic model for the queries
    fields = {}
    if state.web_research:
        fields["web_queries"] = (List[str], Field(default_factory=list, description="Web queries"))
    if state.kb_search:
        fields["kb_queries"] = (List[str], Field(default_factory=list, description="KB queries"))
    if state.excel_search:
        fields["excel_queries"] = (List[str], Field(default_factory=list, description="Excel queries"))
    if not fields:
        return state
    ...
    structured_llm = gpt_4.with_structured_output(DynamicQueries, method="function_calling")
    try:
