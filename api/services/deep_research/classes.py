...
class DataPoint(BaseModel):
    ...
    comparison:     Optional[Dict[str, float]] =    Field(None, description="Comparison data (e.g., industry avg, competitor metrics)")
...
class SectionContent(BaseModel):
    ...
    data_points:        List[DataPoint] =   Field(default_factory=list, description="Key numerical findings extracted from research",)
    key_takeaways:      List[str] =         Field(default_factory=list, description="3-5 main conclusions from this section")
    further_research:   bool =              Field(default=False, description="Whether additional research is needed")
...
class SectionChooser(BaseModel):
    chosen_index: Optional[int] = Field(None, description="Index of the section that best matches `state.update_query`, or null.")

class UpdateQueries(BaseModel):
    queries: List[str] = Field(..., description="Two follow-up queries to research the update.")

class UpdateParagraph(BaseModel):
    paragraph: str = Field(..., description="The paragraph to be updated with new information.")
