...
async def generate_enhanced_html(
    markdown_text: str, title: str, sub_title: str, theme: str = "professional"
) -> str:
    print(
        f"[generate_enhanced_html] Starting enhanced HTML generation with theme: {theme}"
    )
...
"""
    try:
        print(
            "[generate_enhanced_html] Sending detailed prompt to OpenAI GPT-4o mini..."
        )
        ...
        response = await asyncio.to_thread(
            lambda: client.chat.completions.create(
                messages=[
                    {
                        "role": "system",
                        "content": "You are a creative and expert web developer.",
                    },
        ...
        final_html = BASE_HTML_TEMPLATE.format(
            title=title, subtitle=sub_title, content=content_html, **colors
        )

        print(
            f"[generate_enhanced_html] Generated HTML length: {len(final_html)} characters"
        )
        return final_html
...
async def generate_pdf(report_req: ReportRequest):
    ...
    try:
        ...
        return JSONResponse(
            content={"message": "Reports fetched successfully", "data": enhanced_html},
            status_code=200,
        )
    except Exception as e:
