...
interface CostFactor {
  ...
  export function CostFactors({ 
    ...
    // Ensures costFactors is an array, defaulting to empty array if undefined or incorrect type
    const validatedCostFactors: CostFactor[] = Array.isArray(costFactors) 
      ? costFactors 
      : [];
              ...
              >
                ...
                >
                  <h4 className="font-semibold text-lg">
                    {factor.cost_factor_title || "Untitled Factor"}
                  </h4>
                  {/* <span className="text-lg">{openIndex === index ? "-" : "+"}</span> */}
                </div>
                {openIndex === index && (
                  <p className="mt-2 text-gray-300">
                    {factor.cost_factor_description || "No description available."}
                  </p>
                )}
              </div>
            ))
