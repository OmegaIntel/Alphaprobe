...
interface MarketSegmentationProps {
  marketSegmentation?: MarketSegment[]; // Allow optional prop
}

const MarketSegmentation: React.FC<MarketSegmentationProps> = ({
  marketSegmentation = [], // Provide default empty array
}) => {
  // Ensure each item in the array matches the MarketSegment interface
  const validMarketSegmentation = marketSegmentation.filter(
    (segment): segment is MarketSegment =>
  ...
  if (validMarketSegmentation.length === 0) {
    return (
      <div className="text-yellow-500 text-center">
        <p>No valid data available for market segmentation.</p>
      </div>
    );
  }

  // Calculate the total percentage of provided segments
  const totalPercentage = validMarketSegmentation.reduce(
    (sum, segment) => sum + segment.segment_percentage,
    0
  );

  // Check if "Others" is already included in the data
  const hasOthers = validMarketSegmentation.some(
              ...
              >
                ...
                <div>
                  ...
                  <p className="text-[#a8a8a8]">
                    {segment.segment_description || "No description available."}
                  </p>
                </div>
              </li>
            ))}
