...
interface SupplyChainData {
  tier_1_buyers?: string[];
  tier_1_suppliers?: string[];
  tier_2_buyers?: string[];
  tier_2_suppliers?: string[];
}
...
const TierSection: React.FC<TierSectionProps> = ({ title, items = [] }) => {
  return (
    <div className="border rounded-2xl border-[#2b5ba2] bg-[#0D0D0D] p-4 shadow-md">
      ...
      {items && items.length > 0 ? (
        ...
        <div>
          <p className="ml-10 space-y-2 text-[#7a7a7a] p-2">
            No {title} available.
          </p>
        </div>
      )}
