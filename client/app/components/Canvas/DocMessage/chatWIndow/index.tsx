...
import { extractMessageFromOutput, getAnimationOrigin, getChatPosition } from "../../api";
import React, { useEffect, useRef, useState } from "react";
import ChatMessage from "./chatMessage";
//import { sendMessage } from "~/services/HTTPS";
...
export default function ChatWindow({
  ...
  function handleClick() {
    if (value && value.trim() !== "") {
      ...
      setValue("");
      getDocumentReport({title:value, subTitle: "Custom report"})
        .then((res) => {
          if (
            res &&
            res.outputs &&
            Object.keys(res.outputs).length > 0 &&
            res.outputs[0].outputs && res.outputs[0].outputs.length > 0
          ) {
            const flowOutputs: Array<any> = res.outputs[0].outputs;
            if (output_component &&
