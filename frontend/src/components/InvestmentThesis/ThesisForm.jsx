...
import thesisbg from "../../icons/thesisBackground.jpg";
...
export const ThesisForm = ({ questions }) => {
  ...
  const [resetNavigation, setResetNavigation] = useState(false); // State to control navigation reset
  ...
  const handleSubmit = (e) => {
    ...
    setLoading(true);
    setResetNavigation(false); // Prevent navigation reset during loading
    ...
    fetch(`${API_BASE_URL}/api/industries-for-thesis`, {
      ...
      .then((data) => {
        ...
        setResetNavigation(true); // Allow navigation reset after successful response
      })
      .catch((error) => {
  ...
  return (
    <div
      className="relative p-3 h-full w-full bg-cover bg-center"
      style={{ backgroundImage: `url(${thesisbg})` }}
    >
      {/* Overlay */}
