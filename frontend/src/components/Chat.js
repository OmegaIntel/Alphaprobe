...
const Chat = ({ setToken, updateSidebarSessions }) => {
  ...
  const [company, setCompany] = useState(localStorage.getItem('selectedCompany') || ''); // Get company from localStorage or default to empty string
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      const summary = generateSessionSummary([...conversation, { role: 'ai', content: aiResponse }]);
  ...
  const generateSessionSummary = (conversation) => {
    ...
    const lastFewMessages = conversation.slice(-3).map(msg => msg.content).join(' ').slice(0, 50);
    return lastFewMessages.length > 50 ? `${lastFewMessages}...` : lastFewMessages;
  };

  const updateSessionName = async (sessionId, sessionName) => {
    try {
      await axiosInstance.put(`/chat/sessions/${sessionId}/name`, { session_name: sessionName }, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
