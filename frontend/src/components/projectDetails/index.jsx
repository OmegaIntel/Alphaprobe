...
import { createWorkspace, deleteWorkspace, editWorkspace, getWorkspace } from '../../services/currentWorkspace';
    ...
    const addProject = () => {
        ...
        if (isActiveSubCategory === "Current Workspace") {
            ...
            createWorkspace(workspaceData)
                ...
                .catch((e) => {
                    notification.error({ "message": `Something Went Wrong In Saving ${isActiveSubCategory}` });
                });
        }
    ...
    const saveProject = async (id, description) => {
        ...
        if (isActiveSubCategory === "Current Workspace") {
            ...
            editWorkspace(id, updatePayload)
                .then(() => {
                    setToggle((prev) => !prev);
                    notification.success({ "message": `Successfully edited ${isActiveSubCategory}` });
                })
                .catch((e) => {
                    notification.error({ "message": `Something Went Wrong In Updating ${isActiveSubCategory}` });
                })
        }
        ...
        if (isRemove) {
            if (isActiveSubCategory === "Current Workspace") {
                deleteWorkspace(id).then(() => {
                    ...
                    .catch((e) => {
                        notification.error({ "message": `Something Went Wrong In Removing ${isActiveSubCategory}` });
                    })
            }
            else if (isActiveSubCategory === "Knowledge Base") {
                deleteKnowledgebase(id).then(() => {
                    setToggle((prev) => !prev);
                    notification.success({ "message": "Successfully removed!" })
                })
                    .catch((e) => {
