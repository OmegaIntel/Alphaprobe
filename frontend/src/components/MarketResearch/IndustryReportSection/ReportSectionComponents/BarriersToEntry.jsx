...
const BarriersToEntryComponent = ({ barriersToEntry = {} }) => {
  ...
  const barriersPoints = Array.isArray(barriersToEntry.barriers_points) ? barriersToEntry.barriers_points : [];
  const factorsIncreased = Array.isArray(barriersToEntry.factors_increased_barrier) ? barriersToEntry.factors_increased_barrier : [];
  const factorsDecreased = Array.isArray(barriersToEntry.factors_decreased_barrier) ? barriersToEntry.factors_decreased_barrier : [];
        ...
        {barriersPoints.map((point, index) => (
          ...
          >
            <div onClick={() => toggleAccordion(index)} className="flex justify-between items-center cursor-pointer">
              <h5 className="text-lg text-[#b9bbbe] font-medium">{point.barrier_title || "Untitled Barrier"}</h5>
              <span className="text-lg">{openIndex === index ? "-" : "+"}</span>
            </div>
            {openIndex === index && (
              <p className="mt-2 font-normal text-[#a8a8a8]">{point.barrier_description || "No description available."}</p>
            )}
          </div>
        ))}
      </div>
        ...
        <div className="mt-6 w-[30rem] border-[#2b5ba2] border bg-[#0D0D0D] p-3 rounded-2xl">
          ...
          <ul className="list-disc ml-10 space-y-2 text-[#7a7a7a] p-2">
            {factorsIncreased.length > 0 ? (
              factorsIncreased.map((factor, index) => <li key={index}>{factor}</li>)
            ) : (
        ...
        <div className="mt-6 w-[30rem] border border-[#2b5ba2] bg-[#0D0D0D] p-3 rounded-2xl">
          ...
          <ul className="list-disc ml-10 space-y-2 text-[#7a7a7a] p-2">
            {factorsDecreased.length > 0 ? (
              factorsDecreased.map((factor, index) => <li key={index}>{factor}</li>)
            ) : (
              <li>No factors available.</li>
            )}
          </ul>
