...
const TechnologicalChange = ({ technologicalChange }) => {
  ...
  return (
    <div className="rounded-lg p-4 shadow-md text-gray-400">
      <h3 className="text-xl font-semibold mb-4 text-white">Technological Change</h3>

      <div className="mb-4 flex justify-between">
        <p className="text-lg">
          <span className="font-semibold">Technological Change Level: </span>
      ...
      <div className="space-y-4">
        {technologicalChange.technological_change_points.map((point, index) => (
          ...
          >
            ...
            >
              <h4 className="font-semibold text-lg">{point.technological_change_title}</h4>
              {/* <span className="text-lg">{openIndex === index ? "-" : "+"}</span> */}
            </div>
            {openIndex === index && (
              <p className="mt-2 text-gray-300">{point.technological_change_description}</p>
            )}
          </div>
        ))}
      </div>
