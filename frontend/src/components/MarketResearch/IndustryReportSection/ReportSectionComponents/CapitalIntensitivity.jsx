...
const CapitalIntensity = ({ capitalIntensity }) => {
  ...
  return (
    <div className="rounded-lg p-4 shadow-md text-gray-400">
      <h3 className="text-xl font-semibold mb-4 text-white">Capital Intensity</h3>

      <div className="mb-4 flex justify-between">
        <p className="text-lg">
          <span className="font-semibold">Capital Intensity Level: </span>
      ...
      <div className="space-y-4">
        {capitalIntensity.capital_intensity_points.map((point, index) => (
          ...
          >
            ...
            >
              <h4 className="font-semibold text-lg">{point.capital_intensity_title}</h4>
              {/* <span className="text-lg">{openIndex === index ? "-" : "+"}</span> */}
            </div>
            {openIndex === index && (
              <p className="mt-2 text-gray-300">{point.capital_intensity_description}</p>
            )}
          </div>
        ))}
      </div>
