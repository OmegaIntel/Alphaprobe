...
const ScorecardComponent = ({ metrics }) => {
  // Error handling: Check if metrics is provided and is an array
  if (!Array.isArray(metrics) || metrics.length === 0) {
    return <div className="text-red-500 text-center">Error: Invalid or empty metrics data</div>;
  }
  ...
  if (marketMetrics.length === 0) {
    return <div className="text-red-500 text-center">No Market metrics found</div>;
  }
  ...
  const calculateOverallAverageScore = () => {
    const totalScore = marketMetrics.reduce(
      (acc, aspect) =>
        acc + aspect.Scores.reduce((sum, score) => sum + (score.Score || 0), 0), // Ensure Score exists
      0
    );
