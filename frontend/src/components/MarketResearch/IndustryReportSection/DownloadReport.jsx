...
const formatCurrency = (value) => {
  ...
  const styles = StyleSheet.create({
    ...
    section: { marginBottom: 8, flexDirection: 'row', alignItems: 'center', backgroundColor: '#333333', padding: 8, borderRadius: 6 },
    indicator: { width: 4, height: 32, backgroundColor: '#3B82F6', borderRadius: 4, marginRight: 8 },
    textContainer: { flexDirection: 'column' },
    title: { fontSize: 10, color: '#94A3B8', marginBottom: 2 },
    value: { fontSize: 12, color: '#E5E7EB', fontWeight: 'bold' },
  });
// Sample JSON data
const data = {
    "result": [
...
const PdfContent = ({ dataArray }) => (
    <Document>
    {dataArray.map((data, index) => (
      <Page key={index} style={styles.container}>
        <Text style={{ fontSize: 16, marginBottom: 12 }}>Key Statistics - Data Set {index + 1}</Text>
        ...
        <View style={styles.section}>
          ...
          <View style={styles.textContainer}>
            ...
            <Text style={styles.value}>{formatCurrency(data.key_statistics.revenue.revenue_dollars)}</Text>
          </View>
        </View>
        ...
        <View style={styles.section}>
          <View style={styles.indicator}></View>
          <View style={styles.textContainer}>
            <Text style={styles.title}>Historical CAGR (2005-2024)</Text>
            <Text style={styles.value}>{data.key_statistics.revenue.revenue_cagr_historical.revenue_cagr_value}%</Text>
          </View>
        </View>
        ...
        <View style={styles.section}>
          <View style={styles.indicator}></View>
          <View style={styles.textContainer}>
            <Text style={styles.title}>Projected CAGR (2024-2030)</Text>
            <Text style={styles.value}>{data.key_statistics.revenue.revenue_cagr_projected.revenue_cagr_value}%</Text>
          </View>
        </View>
        ...
        <View style={styles.section}>
          <View style={styles.indicator}></View>
          <View style={styles.textContainer}>
            <Text style={styles.title}>Profit Margin</Text>
            <Text style={styles.value}>{data.key_statistics.profit_margins.profit_margins_percentage}%</Text>
          </View>
        </View>
      </Page>
    ))}
...
const  DownloadReport = () => {
    const handleDownload = async () => {
      const blob = await pdf(<PdfContent data={data.result} />).toBlob();
